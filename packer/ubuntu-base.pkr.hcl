...
locals {
  iso_filename = "ubuntu-${var.ubuntu_version}-live-server-amd64.iso"
  vm_name      = "ubuntu-${var.ubuntu_version}-base"
}
...
source "proxmox-iso" "ubuntu" {
  ...
  template_description = "Ubuntu ${var.ubuntu_version} base — built by Packer on ${formatdate("YYYY-MM-DD", timestamp())}"
  os                   = "l26"
  qemu_agent           = true

  # ── ISO ─────────────────────────────────────────────────────────────────────
  iso_url          = "https://releases.ubuntu.com/${var.ubuntu_version}/${local.iso_filename}"
